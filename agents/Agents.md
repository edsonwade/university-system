...
Your task is to design and implement a **complete testing strategy** for this student management system
All required dependencies are already configured (do not add or modify Maven dependencies). including:
 
  1. **Controller tests** using `MockMvc`
  2. **Integration tests** using:
     * Flyway migrations
     * Configuration loaded from `application-test.yml` (the file is currently empty, provide all required configurations)
     * `Testcontainers` for the database
  3. **BDD tests** using **Cucumber** covering all existing services and main business scenarios, following **SaaS-grade best practices**
- **Note**: JUnit 5, PostgreSQL, and Maven.
Assume production-quality code and explain any non-trivial decisions.
